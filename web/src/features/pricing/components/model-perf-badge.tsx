...
function getStatusBarColor(rate: number | null, isOldest: boolean): string {
  if (rate == null) {
    return isOldest ? 'bg-muted-foreground/10' : 'bg-muted-foreground/15'
  }
...
export const ModelPerfBadge = memo(function ModelPerfBadge(
  ...
  const statusBars = [
    { id: 'oldest', height: 'h-2', rate: paddedRates[0] ?? null },
    { id: 'middle', height: 'h-2.5', rate: paddedRates[1] ?? null },
    { id: 'newest', height: 'h-3', rate: paddedRates[2] ?? null },
  ] as const
          ...
          {statusBars.map((bar) => (
            <span
              ...
              className={cn(
                'w-1 rounded-full',
                bar.height,
                getStatusBarColor(bar.rate, bar.id === 'oldest')
              )}
            />
          ))}
        </div>
